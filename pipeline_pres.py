...
def run_full_audit_pres(pdf_path:str, question:str, top_k: int = 5, threshold: float = 0.35) -> dict:
    ...
    Args:
        ...
        chat_history : conversation history (None = premier appel)
...
def run_chat_turn(
    ...
    """
    Effectue un tour de chat supplémentaire en réutilisant les collections
    déjà chargées en mémoire (pas de re-chunking ni re-embedding).
 
    Args:
        session_doc_collection  : Collection éphémère du document (stockée en session API)
        session_rgpd_collection : Collection RGPD persistante (stockée en session API)
        question     : Nouvelle question de l'utilisateur
        chat_history : Historique complet de la conversation jusqu'ici
        top_k, threshold : Paramètres RAG
 
    Returns:
        dict : question, verdict, doc_chunks, law_articles
    """
